...
namespace Simple.Net.Client
{
    public class Client
    {
        ...
        void OnConnect(IAsyncResult asyncResult)
        {
            ...
            {
                Logger.Warn("DB Connect timeout, trying again");
        ...
        void OnReceive(IAsyncResult asyncResult)
        {
            ...
            if (eventType == null)
            {
                Logger.Error($"ERR: Unknown event code [hash: {reader.hashCode}]");
                return;
            }
            if (!Events.ContainsKey(reader.hashCode))
            {
                Logger.Error($"ERR: Got packet with no callback [name: {eventType.Name}, hash: {reader.hashCode}]");
                return;
            }
            ...
            switch (eventHandler)
            {
                ...
                case Action<object> action:
                    action(packetInstance);
                    break;
                case Action<Client> action:
                    action(this);
                    break;
                case Action<Client, object> action:
                    action(this, packetInstance);
                    break;
                default:
                    Logger.Error($"Got unknown event callback for packet! [name: {eventType.Name}, hash: {reader.hashCode}, callback: {eventHandler}]");
                    break;
            }
